...
type BitcaskClient struct {
	...
	dbs    *bitcask_redis.RedisDataStructure
}
...
func set(cli *BitcaskClient, args [][]byte) (interface{}, error) {
	...
	if err := cli.dbs.Set(key, 0, value); err != nil {
		return nil, err
	}
...
func hset(cli *BitcaskClient, args [][]byte) (interface{}, error) {
	...
	res, err := cli.dbs.HSet(key, field, value)
	if err != nil {
		return nil, err
	}
...
func sadd(cli *BitcaskClient, args [][]byte) (interface{}, error) {
	...
	res, err := cli.dbs.SAdd(key, member)
	if err != nil {
		return nil, err
	}
...
func lpush(cli *BitcaskClient, args [][]byte) (interface{}, error) {
	...
	key, value := args[0], args[1]
	res, err := cli.dbs.LPush(key, value)
	if err != nil {
		return nil, err
	}
...
func zadd(cli *BitcaskClient, args [][]byte) (interface{}, error) {
	...
	res, err := cli.dbs.ZAdd(key, utils.Float64FromBytes(score), member)
	if err != nil {
		return nil, err
	}
