...
// RedisDataStucture Redis 数据结构服务
...
/*
	String
	key, value = type + expire + payload(real value)
...
func (rds *RedisDataStructure) Set(key []byte, ttl time.Duration, value []byte) error {
	...
	var expire uint64 = 0
	if ttl != 0 {
		expire += uint64(time.Now().Add(ttl).UnixNano())
	}

	index += binary.PutUvarint(buf[index:], expire)
...
func (rds *RedisDataStructure) Get(key []byte) ([]byte, error) {
	...
	var index = 1
	expire, n := binary.Uvarint(encValue[index:])
	index += n

	// 判断是否过期
	if expire > 0 && expire <= uint64(time.Now().UnixNano()) {
		return nil, nil
	}
...
func (rds *RedisDataStructure) findMetadata(key []byte, dataType redisDataType) (*metadata, error) {
	...
	if err == bitcask.ErrKeyNotFound {
		...
		// 判断过期时间
		if meta.expire != 0 && meta.expire <= uint64(time.Now().UnixNano()) {
			exist = false
		}
	}
